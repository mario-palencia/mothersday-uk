...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; city: string }> }): Promise<Metadata> {
  ...
  const seoKeywords = getPageTypeSEOKeywords(citySlug, 'ideas', locale as Locale);
  const t = await getTranslations({ locale, namespace: 'Metadata' });
  const title = t('cityPageIdeasTitle', { city: cityName });
  const description = t('cityPageIdeasDescription', { city: cityName });
  
  // Combine brand and non-brand keywords
  const allKeywords = [...seoKeywords.brand, ...seoKeywords.nonBrand];
  
  // Build URL with locale
  const basePath = `/${locale}/${citySlug}/valentines-day/ideas/`;
  ...
  return {
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: `${cityName} skyline - Valentine's Day Ideas`,
        },
      ],
    },
    twitter: {
    ...
    alternates: {
      ...
      languages: {
        'en': `https://celebratemothersday.com/en/${citySlug}/valentines-day/ideas/`,
        'es': `https://celebratemothersday.com/es/${citySlug}/valentines-day/ideas/`,
        'fr': `https://celebratemothersday.com/fr/${citySlug}/valentines-day/ideas/`,
        'de': `https://celebratemothersday.com/de/${citySlug}/valentines-day/ideas/`,
        'it': `https://celebratemothersday.com/it/${citySlug}/valentines-day/ideas/`,
        'pt': `https://celebratemothersday.com/pt/${citySlug}/valentines-day/ideas/`,
        'x-default': `https://celebratemothersday.com/en/${citySlug}/valentines-day/ideas/`,
      },
    },
    robots: {
      index: true,
...
export default async function IdeasPage({ params }: { params: Promise<{ locale: string; city: string }> }) {
  try {
    const { locale, city } = await params;
    ...
    // Filter plans for ideas page
    const filteredPlans = filterPlansByPageType(data.all, 'ideas');
    ...
    return (
      <>
        ...
        <section
          aria-label="Valentine's Day Ideas in city"
        ...
        >
          <h2 className="text-xl md:text-2xl font-bold mb-2 text-center text-white drop-shadow-sm">
            Valentine&apos;s Day Ideas in {cityName}
          </h2>
