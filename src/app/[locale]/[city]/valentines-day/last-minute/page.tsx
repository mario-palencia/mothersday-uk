...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; city: string }> }): Promise<Metadata> {
  ...
  const seoKeywords = getPageTypeSEOKeywords(citySlug, 'last-minute', locale as Locale);
  const t = await getTranslations({ locale, namespace: 'Metadata' });
  const title = t('cityPageLastMinuteTitle', { city: cityName });
  const description = t('cityPageLastMinuteDescription', { city: cityName });
  
  // Combine brand and non-brand keywords
  const allKeywords = [...seoKeywords.brand, ...seoKeywords.nonBrand];
  
  // Build URL with locale
  const basePath = `/${locale}/${citySlug}/valentines-day/last-minute/`;
  ...
  return {
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: `${cityName} skyline - Last-Minute Valentine's Plans`,
        },
      ],
    },
    twitter: {
    ...
    alternates: {
      ...
      languages: {
        'en': `https://celebratemothersday.com/en/${citySlug}/valentines-day/last-minute/`,
        'es': `https://celebratemothersday.com/es/${citySlug}/valentines-day/last-minute/`,
        'fr': `https://celebratemothersday.com/fr/${citySlug}/valentines-day/last-minute/`,
        'de': `https://celebratemothersday.com/de/${citySlug}/valentines-day/last-minute/`,
        'it': `https://celebratemothersday.com/it/${citySlug}/valentines-day/last-minute/`,
        'pt': `https://celebratemothersday.com/pt/${citySlug}/valentines-day/last-minute/`,
        'x-default': `https://celebratemothersday.com/en/${citySlug}/valentines-day/last-minute/`,
      },
    },
    robots: {
      index: true,
...
export default async function LastMinutePage({ params }: { params: Promise<{ locale: string; city: string }> }) {
  try {
    const { locale, city } = await params;
    ...
    // Filter plans for last-minute page
    const filteredPlans = filterPlansByPageType(data.all, 'last-minute');
    
    const basePath = `/${locale}/${citySlug}/valentines-day/last-minute/`;
    const canonicalUrl = `https://celebratemothersday.com${basePath}`;
    
    // Prepare plans for structured data (eventDates + priceValue for schema Event)
    ...
    return (
      <>
        ...
        <section
          aria-label="Last minute Valentine's Day in city"
        ...
        >
          <h2 className="text-xl md:text-2xl font-bold mb-2 text-center text-white drop-shadow-sm">
            Last minute Valentine&apos;s Day in {cityName}
          </h2>
    ...
    console.error('Error in LastMinutePage:', error);
