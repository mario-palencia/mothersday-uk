...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; city: string }> }): Promise<Metadata> {
  ...
  const seoKeywords = getPageTypeSEOKeywords(citySlug, 'restaurants', locale as Locale);
  const t = await getTranslations({ locale, namespace: 'Metadata' });
  const title = t('cityPageRestaurantsTitle', { city: cityName });
  const description = t('cityPageRestaurantsDescription', { city: cityName });
  
  // Combine brand and non-brand keywords
  const allKeywords = [...seoKeywords.brand, ...seoKeywords.nonBrand];
  
  // Build URL with locale
  const basePath = `/${locale}/${citySlug}/restaurants/`;
  ...
  return {
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: `${cityName} skyline - Romantic Restaurants`,
        },
      ],
    },
    twitter: {
    ...
    alternates: {
      ...
      languages: {
        'en': `https://celebratemothersday.com/en/${citySlug}/restaurants/`,
        'es': `https://celebratemothersday.com/es/${citySlug}/restaurants/`,
        'fr': `https://celebratemothersday.com/fr/${citySlug}/restaurants/`,
        'de': `https://celebratemothersday.com/de/${citySlug}/restaurants/`,
        'it': `https://celebratemothersday.com/it/${citySlug}/restaurants/`,
        'pt': `https://celebratemothersday.com/pt/${citySlug}/restaurants/`,
        'x-default': `https://celebratemothersday.com/en/${citySlug}/restaurants/`,
      },
    },
    robots: {
      index: true,
...
export default async function RestaurantsPage({ params }: { params: Promise<{ locale: string; city: string }> }) {
  try {
    const { locale, city } = await params;
    ...
    // Filter plans for restaurants page
    const filteredPlans = filterPlansByPageType(data.all, 'restaurants');
    
    const basePath = `/${locale}/${citySlug}/restaurants/`;
    const canonicalUrl = `https://celebratemothersday.com${basePath}`;
    
    // Prepare plans for structured data (eventDates + priceValue for schema Restaurant/Event)
    ...
    return (
      <>
        ...
        <section
          aria-label="Valentine's Day Restaurants in city"
        ...
        >
          <h2 className="text-xl md:text-2xl font-bold mb-2 text-center text-white drop-shadow-sm">
            Valentine&apos;s Day Restaurants in {cityName}
          </h2>
